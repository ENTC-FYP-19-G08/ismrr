...
def text_publish(response):
    global text_id, text
    response_dic = json.loads(response)
    id_ = response_dic["metadata"]["extra"]["id_"]
    new_text = response_dic["results"]["channels"][0]["alternatives"][0]["transcript"]
    if id_ == text_id:
        text += new_text
    else:
    ...
    for frame in frames:
        if frame is not None:
            ...
            if save_path:
                ...
                text_response = speechToText(file_name)
                text_publish(text_response)
