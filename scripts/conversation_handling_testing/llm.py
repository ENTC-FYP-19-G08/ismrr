...
class LLM:
    ...
    def __init__(
        ...
        self.parser = argparse.ArgumentParser(
            prog="chat.py",  # Program name displayed in help message
            description="Simple Llama2 chat example for ExLlamaV2",  # Description of the program
            conflict_handler="error",
            formatter_class=argparse.HelpFormatter,
            add_help=True,  # Whether to include a default help option
        )
        model_init.add_args(self.parser)
        ...
        print(f" -- Prompt format: {self.args.mode}")
        print(f" -- System prompt:")
        print()
        print(self.col_sysprompt + self.system_prompt.strip() + self.col_default)
    ...
    def make_sentence(self, chunk_, flag):
        if flag:
    ...
    def text_to_speech_with_pitch(self, text, pitch_shift, output_path):
        # Initialize the text-to-speech engine
        engine = pyttsx3.init()

        # Set the pitch (rate of speech)
        engine.setProperty("rate", 150)  # Adjust as needed

        # Convert text to speech and save to a temporary file
        temp_output_path = "temp.wav"
        engine.save_to_file(text, temp_output_path)
        engine.runAndWait()
        ...
        # Export the final audio with pitch shift
        shifted_audio.export(output_path, format="wav")
        self.play_wav(output_path)
...
mistral.chat_("where is the analog lab?")
